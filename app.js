...
function renderLearnStudy() {
  ...
  if (!s.revealed) return `<section class="question-card vocab-card"><div class="question-meta"><span>背词回忆 · ${s.index + 1}/${s.ids.length}</span><span>先回忆再看答案</span></div><div class="vocab-front"><span class="eyebrow">原形</span><h1>${esc(v.v1)}</h1><p class="vocab-meaning">${esc(v.zh)}</p><p class="muted">固定搭配提示：<span class="example">${esc(v.phrase)}</span></p></div><div class="recall-grid"><label>现在时<input id="vocab-pres" placeholder="例如：jobber"></label><label>过去时<input id="vocab-past" placeholder="例如：jobbet"></label><label>完成分词<input id="vocab-pp" placeholder="例如：jobbet"></label></div><div class="actions"><button class="button primary" data-vocab-check>检查并显示答案</button><button class="button secondary" data-vocab-reveal>直接显示答案</button></div></section>`;
...
function startVocabSession(total) {
  ...
  state.vocabSession = { ids: [...due, ...review].slice(0, total).map(v => v.id), index: 0, revealed: false, lastResult: null }; saveState(); location.hash = "learnstudy";
}
function checkVocab() {
  const s = state.vocabSession; const v = getVerb(s.ids[s.index]); const values = [document.querySelector("#vocab-pres").value, document.querySelector("#vocab-past").value, document.querySelector("#vocab-pp").value]; const expected = [v.pres, v.past, v.pp]; const correct = values.map((x, i) => normalize(x) === normalize(expected[i]));
...
function finishVocabCard() { const s = state.vocabSession; const id = s.ids[s.index]; state.learned[id] = (state.learned[id] || 0) + 1; s.index++; s.revealed = false; s.lastResult = null; if (s.index >= s.ids.length) { state.vocabSession = null; location.hash = "learn"; } saveState(); render(); }
...
function bindView() {
  ...
  document.querySelector("[data-vocab-check]")?.addEventListener("click", checkVocab);
  document.querySelector("[data-vocab-reveal]")?.addEventListener("click", () => { state.vocabSession.revealed = true; state.vocabSession.lastResult = null; saveState(); render(); });
  document.querySelector("[data-vocab-retry]")?.addEventListener("click", () => { state.vocabSession.revealed = false; state.vocabSession.lastResult = null; saveState(); render(); });
