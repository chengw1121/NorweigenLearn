...
const DEFAULT_STATE = { mistakes: [], attempts: 0, correct: 0, streak: 1, learned: {}, review: {}, questionMastery: {}, session: null, vocabSession: null, lastSummary: null, questionHistory: [], practiceSessions: 0, savedWords: [], readingProgress: {}, dailyProgress: {}, trainingSettings: { dailyQuestionCount: 10 } };
...
function loadState() { try { const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}"); const count = Number(saved.trainingSettings?.dailyQuestionCount); return { ...DEFAULT_STATE, ...saved, trainingSettings: { ...DEFAULT_STATE.trainingSettings, ...(saved.trainingSettings || {}), dailyQuestionCount: [5,10,15,20,30].includes(count) ? count : 10 } }; } catch { return { ...DEFAULT_STATE }; } }
...
function makeQuestion(type, verb, serial) {
  ...
  if (type === "form") { const sentence = serial % 2 ? `${subject.no} ${verb.pres} ${verb.tail} ${habit.no.toLocaleLowerCase("nb-NO")}.` : `${habit.no} ${verb.pres} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.tail}.`; const prompt = serial % 2 ? `${subject.no} ___ ${verb.tail} ${habit.no.toLocaleLowerCase("nb-NO")}.` : `${habit.no} ___ ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.tail}.`; return makeChoice(id, type, "动词四形", prompt, verb, verb.pres, `这是现在时 ${verb.pres}。${habit.zh} 是习惯/频率线索，动词用现在时。`, { translation: `${subject.zh}${habit.zh} ${verb.zh}。`, correctSentence: sentence, answerTranslation: `${subject.zh}${verb.zh}。`, rule: `现在时可表达当前状态、一般事实或习惯；主语换成不同人称，挪威语动词现在时通常不随人称变化。`, rotationVariant: sentenceId(sentence) }); }
  if (type === "modal") { const modals = [{ no:"skal", zh:"计划/将要" },{ no:"vil", zh:"想要" },{ no:"kan", zh:"能够" },{ no:"må", zh:"必须" },{ no:"bør", zh:"应该" }]; const modal = modals[serial % modals.length]; const sentence = `${subject.no} ${modal.no} ${verb.v1} ${verb.tail}.`; return makeChoice(id, type, "情态 + V1", `${subject.no} ${modal.no} ___ ${verb.tail}.`, verb, verb.v1, `${modal.no}（${modal.zh}）已经承担了变位，后面的动作使用原形 ${verb.v1}，不加 -r，也不加 å。`, { translation: `${subject.zh}${modal.zh}${verb.zh}。`, correctSentence: sentence, answerTranslation: `${subject.zh}${modal.zh}${verb.zh}。`, rule: `情态动词 ${modal.no} 后直接接动词原形：${modal.no} + V1。挪威语情态动词本身变位，后面的主要动词不变位。`, rotationVariant: sentenceId(sentence) }); }
  if (type === "past") { const time = PAST_TIMES[serial % PAST_TIMES.length]; const sentence = `${time.no} ${verb.past} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.tail}.`; return makeChoice(id, type, "过去时", `${time.no} ___ ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.tail}.`, verb, verb.past, `${time.no} 明确表示过去，使用过去时 ${verb.past}。`, { translation: `${time.zh}${subject.zh}${verb.zh}。`, correctSentence: sentence, answerTranslation: `${time.zh}${subject.zh}${verb.zh}。`, rule: `明确过去时间词（如 i går、forrige uke）通常搭配过去时。主语和时间短语变化不会改变过去时词形。`, rotationVariant: sentenceId(sentence) }); }
  if (type === "perfect") { const sentence = `${subject.no} har ${verb.pp} ${verb.tail}.`; const time = ["allerede","nettopp","flere ganger","i dag"][serial % 4]; return makeChoice(id, type, "har + 完成分词", `${subject.no} har ${time} ___ ${verb.tail}.`, verb, verb.pp, `har 后使用完成分词 ${verb.pp}；主语变化时 har 仍保持不变。`, { translation: `${subject.zh}已经/刚刚${verb.zh}。`, correctSentence: sentence, answerTranslation: `${subject.zh}已经${verb.zh}。`, rule: `现在完成时结构是主语 + har + 完成分词。副词（allerede、nettopp）可放在完成分词前，但本题空格仍填完成分词。`, rotationVariant: sentenceId(`${subject.no} har ${time} ${verb.pp} ${verb.tail}`) }); }
  if (type === "infinitive") { const starters = [{ no:"Jeg liker å", zh:"我喜欢" },{ no:"Hun prøver å", zh:"她试着" },{ no:"Vi ønsker å", zh:"我们希望" },{ no:"De trenger å", zh:"他们需要" }]; const starter = starters[serial % starters.length]; const sentence = `${starter.no} ${verb.v1} ${verb.tail}.`; return makeChoice(id, type, "å + V1", `${starter.no} ___ ${verb.tail}.`, verb, verb.v1, `${starter.no.split(" ").slice(-1)[0]} 是变位动词/结构，后面的动作使用 å + 原形 ${verb.v1}。`, { translation: `${starter.zh}${verb.zh}。`, correctSentence: sentence, answerTranslation: `${starter.zh}${verb.zh}。`, rule: `两个动词连用时，liker / prøver / ønsker / trenger 后的第二个动词常用 å + V1；情态动词后则不加 å。`, rotationVariant: sentenceId(sentence) }); }
  ...
  if (type === "correction") { const time = FUTURE_TIMES[serial % FUTURE_TIMES.length]; const modal = ["skal","vil","kan","må"][serial % 4]; const answer = `${time.no} ${modal} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.v1} ${verb.tail}`; const wrong = `${time.no} ${modal} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.pres} ${verb.tail}`; return { id, type: "input", category: type, skill: "改错", prompt: `改正句子：${wrong}。`, answers: [answer, answer.replace(`${time.no} ${modal} ${subject.no.toLocaleLowerCase("nb-NO")} `, `${subject.no} ${modal} `).replace(` ${verb.tail}`, ` ${verb.tail} ${time.no.toLocaleLowerCase("nb-NO")}`)], answer, verbId: verb.id, explanation: `错误在于 ${modal} 后误用了现在时 ${verb.pres}；应使用原形 ${verb.v1}。`, translation: `改正后表达“${time.zh}${subject.zh}${verb.zh}”。`, correctSentence: `${answer}.`, answerTranslation: `${time.zh}${subject.zh}${verb.zh}。`, rule: `情态动词 ${modal} 承担了时态信息，后面的动词用 V1。句首时间短语后还要遵守 V2：变位动词 ${modal} 紧接时间成分。`, rotationVariant: sentenceId(wrong) }; }
  const sentence = `${subject.no} ${verb.pres} ${verb.tail} ${habit.no.toLocaleLowerCase("nb-NO")}.`;
  return { id, type: "free", category: type, skill: "自由输出", prompt: `用挪威语写一句关于${subject.zh}自己的句子，使用动词“${verb.v1}”，可加入时间“${habit.no.toLocaleLowerCase("nb-NO")}”。`, verbId: verb.id, target: verb.v1, example: sentence, translation: `写一句使用 ${verb.v1} 的个人化表达。`, correctSentence: sentence, answerTranslation: `${subject.zh}${habit.zh}${verb.zh}。`, rotationVariant: sentenceId(sentence) };
}
function makeV2Question(serial, verb = VERBS[0]) { const mode = serial % 3; const subject = SUBJECTS[Math.floor(serial / 3) % SUBJECTS.length];
  if (mode === 0) { const time = FUTURE_TIMES[Math.floor(serial / 5) % FUTURE_TIMES.length]; const modal = ["skal","vil","kan"][Math.floor(serial / 2) % 3]; const first = `${time.no} ${modal} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.v1} ${verb.tail}`; const second = `${subject.no} ${modal} ${verb.v1} ${verb.tail} ${time.no.toLocaleLowerCase("nb-NO")}`; return { id:`v2-${verb.id}-${serial}`, type:"input", category:"v2", skill:"V2 语序", prompt:`把词语组成句子（注意 V2）：${time.no} / ${subject.no.toLocaleLowerCase("nb-NO")} / ${modal} / ${verb.v1} / ${verb.tail}`, answers:[first,second], answer:first, verbId:verb.id, explanation:`时间放句首时，${modal} 必须紧跟在时间成分后，主语 ${subject.no.toLocaleLowerCase("nb-NO")} 放在它后面。时间也可以放句末，形成另一种自然语序。`, translation:`${time.zh}${subject.zh}${verb.zh}。`, correctSentence:`${first}.`, answerTranslation:`${time.zh}${subject.zh}${verb.zh}。`, rule:`挪威语主句遵守 V2：句首时间成分算第一位，变位动词 ${modal} 位于第二位。`, rotationVariant:sentenceId(first) }; }
  if (mode === 1) { const time = PAST_TIMES[Math.floor(serial / 3) % PAST_TIMES.length]; const first = `${time.no} ${verb.past} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.tail}`; const second = `${subject.no} ${verb.past} ${verb.tail} ${time.no.toLocaleLowerCase("nb-NO")}`; return { id:`v2-${verb.id}-${serial}`, type:"input", category:"v2", skill:"V2 语序", prompt:`用过去时组成自然句：${time.no} / ${subject.no.toLocaleLowerCase("nb-NO")} / ${verb.past} / ${verb.tail}`, answers:[first,second], answer:first, verbId:verb.id, explanation:`时间放句首时，变位动词 ${verb.past} 仍占第二位；主语随之放在动词后。`, translation:`${time.zh}${subject.zh}${verb.zh}。`, correctSentence:`${first}.`, answerTranslation:`${time.zh}${subject.zh}${verb.zh}。`, rule:`V2 不等于“第二个单词”，而是主句第一个成分后接变位动词。`, rotationVariant:sentenceId(first) }; }
  const time = HABIT_TIMES[Math.floor(serial / 4) % HABIT_TIMES.length]; const first = `${time.no} ${verb.pres} ${subject.no.toLocaleLowerCase("nb-NO")} ${verb.tail}`; const second = `${subject.no} ${verb.pres} ${verb.tail} ${time.no.toLocaleLowerCase("nb-NO")}`; return { id:`v2-${verb.id}-${serial}`, type:"input", category:"v2", skill:"V2 语序", prompt:`用习惯表达组成自然句：${time.no} / ${subject.no.toLocaleLowerCase("nb-NO")} / ${verb.pres} / ${verb.tail}`, answers:[first,second], answer:first, verbId:verb.id, explanation:`${time.zh} 放在句首时，变位动词 ${verb.pres} 紧随其后；若主语放句首，则动词在主语后。`, translation:`${subject.zh}${time.zh}${verb.zh}。`, correctSentence:`${first}.`, answerTranslation:`${subject.zh}${time.zh}${verb.zh}。`, rule:"主句 V2 规则按成分数位置：句首时间状语是第一成分，变位动词必须是第二成分。", rotationVariant:sentenceId(first) };
}
